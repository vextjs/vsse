...
### Changed
- **默认配置调整** ⚡:
  - `withHeartbeat` 默认值从 `true` 改为 `false`（降低复杂度，按需启用）
  - `maxListeners` 默认值从 `1000` 降低到 `100`（更合理的上限，更容易发现监听器泄漏）
  - **破坏性变更**: 如果您依赖自动心跳检测，请显式设置 `withHeartbeat: true`

### Documentation
- **澄清 idleTimeout 机制**: 明确说明 `idleTimeout` 仅在"无任何监听器"时生效，有监听器时不会因超时断开
- **新增示例文件**: `examples/idle-and-heartbeat-scenarios.js` 展示不同场景下的配置和行为
